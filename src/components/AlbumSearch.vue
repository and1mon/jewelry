    ...
    <div v-if="recentAlbums.length > 0 && !hasSearched" class="space-y-2">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        >
          ...
          />
          <div v-else class="w-8 h-8 bg-gray-300 dark:bg-gray-600 rounded flex items-center justify-center flex-shrink-0">
            <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" />
            </svg>
          </div>
          <div class="flex flex-col min-w-0 max-w-40">
            <span class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ album.title }}</span>
            <span class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ album.artist }}</span>
          </div>
          <!-- Remove button -->
          <button
